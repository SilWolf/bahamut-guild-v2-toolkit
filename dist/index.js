	...
	/******/ var __webpack_modules__ = {
		/***/ 776: /***/ function (
			...
			const BHGV2Core = ({ plugins, library }) => {
					...
					_plugin.onMutateState = (newValue) => {
						...
						if (newValue.latestComments && newValue.latestComments.length > 0) {
							...
							if (gsn && sn && CommentList) {
								...
								let oldCommentIndex = Array.from(
									CommentList.children
								).findIndex(
									(e) =>
										parseInt(e.getAttribute('data-csn')) >=
								...
								for (
									...
									if (
										...
										const _replyContentUser = newComment.element.querySelector(
											'.reply-content__user'
										)
										if (_replyContentUser) {
									...
									while (oldCommentIndex < 9999) {
										...
										const oldCommentElement =
											CommentList.children[oldCommentIndex]
										const oldCommentId = parseInt(
											oldCommentElement.getAttribute('data-csn')
										)
				...
				_dom.EditorContainerFooter.classList.add(
					'bhgv2-editor-container-footer'
				)
				...
				const _handleSubmitConfigForm = (event, options) => {
					...
					const newConfig = Array.from(
						form.querySelectorAll('input[data-config-key]')
					).reduce((prev, element) => {
...
`
					...
					if (newValue[`${_plugin.prefix}:isEnable`] !== undefined) {
						...
						if (isEnabled === false) {
							...
							const timeoutFn = async () => {
								...
								const { comments, commentCount: newCommentsCount } =
		...
		/***/ 353: /***/ (__unused_webpack_module, exports) => {
			/*******************************************************************************************
			 *
		...
		`,
				...
				const _saveWordMap = (_newValue) => {
					localStorage.setItem(
						`${_plugin.prefix}-WordMap`,
						JSON.stringify(_newValue)
					)
					_WORD_MAP = _newValue
				}
	...
	`)[0]
		...
		`)[0]
				...
				const QuickInputConfigAddButton =
					QuickInputConfigContainer.querySelector(`#${_plugin.prefix}-add`)
				QuickInputConfigAddButton.addEventListener('click', () =>
					QuickInputConfigTBody.append(_createRow())
				)
				const QuickInputConfigCloseButton =
					QuickInputConfigContainer.querySelector(`#${_plugin.prefix}-close`)
				QuickInputConfigCloseButton.addEventListener('click', () =>
					QuickInputConfigBackdrop.classList.toggle('active', false)
				)
				const QuickInputConfigSaveButton =
					QuickInputConfigContainer.querySelector(`#${_plugin.prefix}-save`)
				...
				const _formatCarbonTrailingHTML = (content) =>
					`<code>TAB</code> ${content}`
				const _findKeyWordPair = (text, { exactMatch } = {}) => {
					if (!exactMatch) {
						const key = Object.keys(_WORD_MAP).find((_key) =>
							_key.startsWith(text)
						)
