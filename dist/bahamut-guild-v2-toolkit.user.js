	...
	/******/ var __webpack_modules__ = {
		/***/ 776: /***/ function (
			...
			const BHGV2Core = ({ plugins, library }) => {
						...
						if (newValue.latestComments && newValue.latestComments.length > 0) {
							...
							if (gsn && sn && CommentList) {
								const _createCommentElement = (payload) => {
									if (!payload.position) {
										payload.position =
											CORE.DOM.CommentList?.children.length + 1 || 1
									}
									// 生成comment的element
									const newElement = $(
										nunjucks.render('comment.njk.html', {
											...
											youtubeParameterMatcher:
												GuildTextUtil.youtubeParameterMatcher,
									...
									const _replyContentUser = newElement.querySelector(
										'.reply-content__user'
									)
									if (_replyContentUser) {
										newElement.setAttribute(
											'data-user',
											_replyContentUser.textContent
										)
										newElement.setAttribute(
											'data-userid',
											_replyContentUser.href.split('/').pop()
										)
									}
									const [positionSpan, ctimeSpan] = newElement.querySelectorAll(
									...
									const _replyContentContent = newElement.querySelector(
										'.reply-content__cont'
									)
									...
									if (_storedComment) {
										...
										if (_newComment.id === _storedComment.id) {
											...
											if (_storedComment.element && _newComment.payload) {
												...
												const _newElement = _createCommentElement(
													_newComment.payload
												)
												...
												if (
													_storedComment.payload.text !==
													_newComment.payload.text
												) {
										...
										const _replyContentUser = _newComment.element.querySelector(
											'.reply-content__user'
										)
										...
										const [positionSpan, ctimeSpan] =
											_newComment.element.querySelectorAll(
												'.reply_right span.reply_time'
											)
										if (positionSpan) {
											positionSpan.classList.add('bhgv2-comment-position')
										...
										const _replyContentContent =
											_newComment.element.querySelector('.reply-content__cont')
										if (_replyContentContent) {
											const _replayContentExtra = document.createElement('div')
											_replayContentExtra.classList.add(
												'reply-content__override'
											)
											_replyContentContent.insertAdjacentElement(
												'afterend',
												_replayContentExtra
										...
										console.warn(
											'something wrong with a new comment',
											_newComment
										)
										_newCommentIndex++
									}
								...
								setTimeout(() => {
									...
									for (const CTime of CTimes) {
										...
										if (ctime) {
											const humanString =
												display_helper_1.convertCTimeToHumanString(ctime)
				...
				_dom.EditorContainer =
					_dom.CommentListOuter.getElementsByClassName('c-reply__editor')[0]
				...
				_dom.Editor =
					_dom.EditorContainer.getElementsByClassName('reply-input')[0]
				_dom.Editor.classList.add('bhgv2-editor')
				const oldEditorTextarea =
					_dom.Editor.getElementsByTagName('textarea')[0]
				_dom.EditorTextareaWrapper = document.createElement('div')
				_dom.EditorTextareaWrapper.classList.add(
				...
				_dom.EditorContainerReplyContentFooterLeft.append(
					_dom.ConfigPanelStatus
				)
				_dom.EditorContainerReplyContentFooterRight.append(
					_dom.ConfigPanelSwitch
				)
				...
				const _handleSubmitConfigForm = (event, options) => {
					...
					const newConfig = Array.from(
						form.querySelectorAll('[data-config-key]')
					).reduce((prev, element) => {
				...
				GuildComment.commentEdit = (gsn, postSn, commentSn) => {
					...
					const _overrideArea = _element.querySelector(
						'.reply-content__override'
					)
					if (!_overrideArea) {
						return
				...
				_initialCommentListInterval = setInterval(() => {
					...
					if (_CommentListOuter) {
						const commentCount =
							parseInt(_CommentListOuter.getAttribute('data-comment-count')) ||
							0
						if (commentCount === 0) {
						...
						if (_CommentList) {
							...
							const _newComments = Array.from(_CommentList.children).map(
								(element, index) => ({
...
`
		...
		/***/ 507: /***/ (__unused_webpack_module, exports) => {
			Object.defineProperty(exports, '__esModule', { value: true })
			exports.default = `
	.main-container_wall-post_header {
		display: flex;
...
`
		...
		/***/ 201: /***/ (__unused_webpack_module, exports) => {
			Object.defineProperty(exports, '__esModule', { value: true })
		...
		/***/ 503: /***/ (
			...
			const BHGV2_AutoRefresh = (core) => {
				...
				_plugin.configLayout = [
					...
					[
						`${_plugin.prefix}:notification`,
						`${_plugin.prefix}:notificationSound`,
					],
				]
					...
					if (newValue[`${_plugin.prefix}:isEnable`] !== undefined) {
						...
						if (isEnabled === false) {
							...
							const timeoutFn = async () => {
								...
								let _interval =
									parseInt(
										newValue[`${_plugin.prefix}:interval`] ||
		...
		/***/ 340: /***/ (__unused_webpack_module, exports) => {
			/*******************************************************************************************
			 *
			 *  BHGV2_DarkMode - 黑闇模式
		...
		/***/ 115: /***/ (__unused_webpack_module, exports) => {
			/*******************************************************************************************
			 *
			 *  BHGV2_Dense - 密集介面
			...
			const BHGV2_Dense = (core) => {
				...
				_plugin.configLayout = [
					...
					[
						`${_plugin.prefix}-smallerImage`,
						`${_plugin.prefix}-smallerImageHover`,
					],
				...
				_plugin.onMutateState = ({ latestComments }) => {
					if (latestComments) {
						latestComments.forEach((comment) => {
							...
							const _tagButton = element.querySelector(
								'button.reply-content__tag'
							)
							const _contentUser = element.querySelector(
								'a.reply-content__user'
							)
							if (!_tagButton || !_contentUser) {
								return
							}
							const _clonedTagButton = _tagButton.cloneNode(false)
							_clonedTagButton.classList.add(
		...
		`,
				...
				const _buttonHighlightAllAfterMyLastComment =
				...
				const _buttonUnhighlightAllBeforeMyLastComment =
					document.createElement('a')
				...
				_buttonUnhighlightAllBeforeMyLastComment.addEventListener(
					'click',
					(e) => {
						e.preventDefault()
		...
		/***/ 739: /***/ (__unused_webpack_module, exports) => {
			/*******************************************************************************************
			 *
		...
		/***/ 285: /***/ (__unused_webpack_module, exports) => {
			...
			const BHGV2_NotifyOnTitle = (core) => {
				...
				const _getNofityCount = () => {
					...
					return ['topBar_light_0', 'topBar_light_1', 'topBar_light_2'].reduce(
						(prev, name, i) => {
							if (!doCountBools[i]) {
								return prev
							}
				...
				_plugin.onMutateConfig = (newValue) => {
					if (newValue[`${_plugin.prefix}-isEnable`] === true && !_observer) {
						...
						newValue[`${_plugin.prefix}-isEnable`] === false &&
						_observer
					) {
					...
					if (newValue[`${_plugin.prefix}:isEnable`] !== undefined) {
						...
						if (dom) {
							dom.classList.toggle(
								...
								newValue[`${_plugin.prefix}:isEnable`]
							)
						}
					}
				}
